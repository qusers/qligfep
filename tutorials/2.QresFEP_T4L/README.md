...
This can be handled with several solutions - the one we regularly use makes use of [Schrödinger's "Protein Preparation Wizard" software] 
(https://www.schrodinger.com/life-science/learn/white-papers/protein-preparation-workflow/). In addition to the previous, this solution will add hydrogen atoms - including optimized protonation states for ionizable residues based on pKa predictions - and to any existing structural water molecules - all
...
yet the forcefield we will use, [OPLS-AA/M] (https://zarbi.chem.yale.edu/oplsaam.html) format, with different protonation models for histidine name as either HID, HIE, or HIP . In some cases, we might also want to remove some
...
- By turning on the -d DUAL flag, the dual/hybrid topology FEP protocol will be utilised (check reference [Koenekoop et al., 2025] (https://www.nature.com/articles/s42004-025-01771-0); this is the recommended protocol, and also allows all amino acids mutations.
Note that, if this flag is not specifically activated, the single topology FEP protocol for annihilation to Alanine will be used, as implemented in the original version of QresFEP reported in [Jespers et al., 2019] (https://pubs.acs.org/doi/full/10.1021/acs.jctc.9b00538).
...
- The -ts TIMESTEP argument determines the time step, which is either 1 or 2 fs,
in which case the value of the SHAKE algorithm dynamically adjusted accordingly.
...
- Optionally, the location of the Qprep executables can be indicated with the -P PREPLOCATION flag (otherwise the settings.py will point to the location of the Q executables).
...
We will initially test that we can run the 10 replicate simulations corresponding to one leg of the thermodynamic cycle of one mutation. Place yourselt into one of the FEP directories and simply run the submit command therein:
    ...
    cd FEP_LEU39ALA/protein
    bash FEP_submit.sh

If this submits 10 simulations, we are basically done!

In principle, you should do this for all mutations times two (i.e. protein and tripeptide directories therein). For our convenience, we can simply use a bach script that does this iteration for us. We provide one example, but feel free to design such a script in any way that you seem fit four your specific case. For this tutorial we can use the example script submit.sh:

    bash submit.sh

And you should have 9 (mutations) x 2 (legs) x 10 (replicas) = 180 jobs submitted. Each job should take around 2h time using standard 8 or 16 cores per job. Tine this (on settings.py for your cluster entry) to optimize the use of your cluster and have all the mutations done as quick as possible on your cluster. 
...
- Optionally, we can choose to indicate the cluster of analysis
with the -C CLUSTER flag, 
...
The following should be printed out to your terminal (scroll the horizontal bar for an explanatino of each line entry):
...
For each mutation, the free energy difference between the protein and the reference tripeptide systems, equals to the relative folding free energy shift, expressed as ΔΔG, due to that mutation. We can calculate this as follows:
