...
def pdb_parse_in(line, include=('ATOM','HETATM')):
    ...
    if line.startswith(include):
        ...
        at_entry.append(line[22:26])       #  6 Residue sequence number
...
def pdb_parse_out(line, include=('ATOM','HETATM')):
    """
    Takes a list and parses it into a pdb writeable line
    """  
    if len(line[2]) <= 3: 
        line = '{:6s}{:5s}  {:3s}{:1s}{:4s}{:1s}{:4s}{:1s}   '\
               '{:8.3f}{:8.3f}{:8.3f}{:6.2f}{:6.2f}          {:>2s}{:2s}'.format(*line)
        
            
    elif len(line[2]) == 4: 
        line = '{:6s}{:5s}  {:4s}{:1s}{:4s}{:1s}{:4s}{:1s}   '\
               '{:8.3f}{:8.3f}{:8.3f}{:6.2f}{:6.2f}          {:>2s}{:2s}'.format(*line)
