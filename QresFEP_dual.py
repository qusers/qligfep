...
class Run(object):
    ...
    def __init__(self, cofactor, mutation, include, forcefield, windows,
        ...
        # If ligands or ions are present
        self.cofactor = cofactor
        # Check whether all required files are there:
        required = ['protein.pdb', 'water.pdb', 'protPREP.log']
        if self.cofactor:
            required.extend([filename + ext for filename in self.cofactor for ext in ['.pdb', '.lib', '.prm']])
        if not all(os.path.exists(filename) for filename in required):
            raise Exception(f"ERROR: {' '.join(required)} are required files, exiting now.")
        
        self.windows = s.WINDOWS if windows is None else int(windows)
        self.sampling = s.SAMPLING if sampling is None else sampling
        self.temperature = s.TEMPERATURE if temperature is None else temperature
        self.replicates = s.REPLICATES if replicates is None else replicates
        ...
        # Match mutation residue atoms to their PDB atom number
        self.atoms = {atom: 0 for atom in ['CA', 'HA', 'ha', 'HA2', 'ha2', 'HA3', 'ha3',
                                           'CB', 'cb', 'HB', 'hb', 'HB1', 'hb1', 'HB2', 'hb2', 'HB3', 'hb3',
                                           'CG', 'cg', 'CG1', 'cg1', 'CG2', 'cg2', 'OG', 'og', 'OG1', 'og1', 'SG', 'sg',
                                           'HG', 'hg', 'HG1', 'hg1', 'HG2', 'hg2', 'HG3', 'hg3', 'HG11', 'hg11', 'HG12', 'hg12', 'HG13', 'hg13', 'HG21', 'hg21', 'HG22', 'hg22', 'HG23', 'hg23',
                                           'CD', 'cd', 'CD1', 'cd1', 'CD2', 'cd2', 'ND1', 'nd1', 'ND2', 'nd2', 'OD1', 'od1', 'OD2', 'od2', 'SD', 'sd',
                                           'HD', 'hd', 'HD1', 'hd1', 'HD2', 'hd2', 'HD3', 'hd3', 'HD11', 'hd11', 'HD12', 'hd12', 'HD13', 'hd13', 'HD21', 'hd21', 'HD22', 'hd22', 'HD23', 'hd23',
                                           'CE', 'ce', 'CE1', 'ce1', 'CE2', 'ce2', 'CE3', 'ce3', 'OE1', 'oe1', 'OE2', 'oe2', 'NE', 'ne', 'NE1', 'ne1', 'NE2', 'ne2',
                                           'HE', 'he', 'HE1', 'he1', 'HE2', 'he2', 'HE3', 'he3', 'HE21', 'he21', 'HE22', 'he22',
                                           'CZ', 'cz', 'CZ2', 'cz2', 'CZ3', 'cz3', 'NZ', 'nz',
                                           'HZ', 'hz', 'HZ1', 'hz1', 'HZ2', 'hz2', 'HZ3', 'hz3',
                                           'CH2', 'ch2', 'OH', 'oh', 'NH1', 'nh1', 'NH2', 'nh2',
                                           'HH', 'hh', 'HH2', 'hh2', 'HH11', 'hh11', 'HH12', 'hh12', 'HH21', 'hh21', 'HH22', 'hh22']}
        
        # Store replacement values for INPUt files' placeholders
        ...
        # Set the requested lambda state for equilibration
        self.replacements['EQ_LAMBDA'] = '1.000 0.000' if self.start in ['1', '0'] else '0.500 0.500' if self.start == '0.5' else self.replacements['EQ_LAMBDA']
