...
#                    print(center[0])
    ...
    def spheremaker(self):
        ...
        final2={}
        n =1
        #reordrind the new pdb file, so as to have only numbers only to decimal system.
        with open('try53.pdb') as infile, \
             open('try60.pdb', 'w') as outfile5:

             for line5 in infile:
                line5 = IO.pdb_parse_in(line5)
                if type(line5) == list:
                    resnum = line5[6]
                    resname = line5[4]
                    dict_key2=()
                    dict_key2=(resname,)#+str(resnum)
                    resnum=resnum.strip()
                    dict_key2=dict_key2+(resnum,)

                    if dict_key2 not in final2:
                        final2[dict_key2] =str(n)
                        n = n + 1

        #Write the final pdb file
        with open('try53.pdb') as infile, \
             open('test_pdb.pdb', 'w') as outfile6:
            for line5 in infile:
                line5 = IO.pdb_parse_in(line5)
                if type(line5) == list:
                    resnum = line5[6]
                    ...
                    line5[6] = final2[dict_key2]
                    outline3 = IO.pdb_parse_out(line5)

                    outfile6.write(outline3 + '\n')
        

    def prepwizard_parse(self):
        if self.origin == 'gromacs':
#            print('work')
            with open('test_pdb.pdb') as infile, \
                 open('test_pdb.pdb'[:-4] + '_noH.pdb', 'w') as outfile:
                for line in infile:
                    tmp = line
